...
public:
  ...
  void Path(const CString& val) { m_Path = val; }
  void Path(const TCHAR* szPath)
  {
    if (szPath)
      m_Path = szPath;
  }
	...
	void Delimiter(const CString& val) { m_strDelimiter = val; }
...
protected:
  ...
  CRect GetItemRect(CrumbBarItem* pItemChosen);
