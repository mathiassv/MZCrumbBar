/* MZCrumbBar.cpp
	...
	MZCrumbBar Ctrl - Created by Mathias Svensson (C) Copyright 2009-2012 - ( http://result42.com/projects/MZCrumbBar )

  History 

  v1.5  2012-05-03  The in-place edit controller caused a crash if it lost focus before it was completely created.

  v1.4  2011-02-25  Added CRect MZCrumbBar::GetItemRect (CrumbBarItem* pItemChosen) - Provided by Iain Clarke

  v1.3  2010-11-13  Added flag CRU_DONOTSTRIPDELIMITER that will keep the double delimiter. 
  ...
  v1.2  2010-04-15  Added improvements/fixes provided by Iain Clarke
  ...
  to maintain a single distribution point for the source code. If you fix any bug or add enhancements please send
...
namespace
{
  ...
  MZCrumbBar_InplaceEditCtrl::~MZCrumbBar_InplaceEditCtrl()
  {
  }
    ...
    if(m_bCreated == false)
    {
      // OnKillFocus is called before OnCreate is completed. 
      // So send the KILLFOCUS using post so it is received after OnCreate is done 
...
{
	CrumbBarItem* pPreItem = nullptr; // Item before the start Item

  if (m_vCrumbItems.empty())
    return nullptr;

  if (m_DrawState.pStartItem == nullptr)
    return nullptr;
...
// Function added by Iain Clarke, 2011-02-25
// Tries to return the client window coordinates of a given crumb item
CRect MZCrumbBar::GetItemRect (CrumbBarItem* pItemChosen)
{
  // Make a rectangle, 0 width, full height.
  CRect rcReturn;
  GetClientRect (&rcReturn);
  rcReturn.right = rcReturn.left = -1;
  ...
  for(std::vector<CrumbBarItem*>::iterator it = m_vCrumbItems.begin(); it != m_vCrumbItems.end(); ++it)
  {
    ...
    if(bFoundStartItem)
    {
      ...
      // Check if we're at the chosen item yet
      if (*it == pItemChosen)
      {
        rcReturn.left = nSize;
        rcReturn.right = rcReturn.left + nSpacing + pItem->nWidth;
        return rcReturn;
      }
...
bool MZCrumbBar::BuildCrumbbar(CString path)
{
  ...
  while (!path.IsEmpty())
  {
    ...
    if (nFind < 0)
    {
			if(!(m_dwStyleEx & CRU_DONOTSTRIPDELIMITER))
...
ULONG MZCrumbBar::GetGestureStatus(CPoint /*ptTouch*/)
{
  return 0;
 // CrumbBarItem* pCrumbItem = HitTestItem(ptTouch);
 // if(pCrumbItem)
...
CWnd* MZCrumbBar::CreateInplaceEditCtrl(const CString& text)
{
  ...
  if(pEdit->Create(dwStyle, rc, this, 0x1233) == FALSE)
    return NULL;
...
BOOL MZCrumbBar::OnSetCursor(CWnd* /*pWnd*/, UINT /*nHitTest*/, UINT /*message*/)
{
	...
	static HCURSOR  hcArrowCursor = ::LoadCursor(NULL, IDC_ARROW);

	HCURSOR hc = GetCursor();
	if( hc != hcArrowCursor  )
		SetCursor( hcArrowCursor  );
